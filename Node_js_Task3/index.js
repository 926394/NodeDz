...
// Эта часть закомичена по ненадобности, но для наглядности
// fs.writeFileSync(
//   path.join(__dirname, "countLis.json"),
//   JSON.stringify(countLis, null, 2)
// );
...
app.get("/", (req, res) => {
  ...
  res.send(
    `<h1>Корневая страница</h1><p>Просмотров: ${countLis.count1}</p><a href="/about">Ссылка на страницу /about</a>`
  );
});
...
app.get("/about", (req, res) => {
  ...
  res.send(
    `<h1>Страница about</h1><p>Просмотров: ${countLis.count2}</p><a href="/">Ссылка на страницу /</a>`
  );
});
